...
class TestRecipee extends Component {
    ...
    componentDidMount() {
        fetch(`https://api.spoonacular.com/recipes/${this.props.match.params.id}/information?apiKey=adc6b8b2a9dd4856a79419fdcec21e21`)
            .then(response => response.json())
            .then(json => {
                this.setState({ data: json });
                this.setState({ servings: json.servings, IngredientData: json.extendedIngredients }, this.handleIngredientAmount);
            })
    }
    ...
    handleMinus = () => {
        if (this.state.servings > 1) {
            this.setState({ servings: this.state.servings - 1 } ,this.handleNewAmount);
        }
    }
    handlePlus = () => {
        this.setState({ servings: this.state.servings + 1 } ,this.handleNewAmount);
